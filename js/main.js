...
jQuery(function($) {'use strict';
	...
	$(document).ready(function() {
		...
		$('#days_together').data('digit', parseInt(DayDiff(new Date(2014, 11, 26), Date.now())));
	});
	...
	form.submit(function(event){
		...
		$.ajax({
			type: 'POST',
			data: {name: $('#NameTB').val(), email: $('#EmailTB').val(), subject: $('#SubjectTB').val(), message: $('#MessageTB').val()},
	...
	//Google Map
	var rlatitude = $('#reception-map').data('latitude');
	var rlongitude = $('#reception-map').data('longitude');
	function initialize_reception_map() {
		var rmyLatlng = new google.maps.LatLng(rlatitude,rlongitude);
		var rmapOptions = {
			zoom: 16,
			scrollwheel: false,
			center: rmyLatlng
		};
		var rmap = new google.maps.Map(document.getElementById('reception-map'), rmapOptions);
		var rmarker = new google.maps.Marker({
			position: rmyLatlng,
			map: rmap
		});
	}
	google.maps.event.addDomListener(window, 'load', initialize_reception_map);
